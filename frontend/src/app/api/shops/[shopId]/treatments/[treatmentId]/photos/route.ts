...
export const POST = withShopAuth<{ shopId: string; treatmentId: string }>(
  async (req, params, member) => {
    ...
    const filePath = `${prefix}/${params.treatmentId}/${randomUUID()}.${ext}`;
    ...
    if (thumbnail && mediaType === "video") {
      ...
      const thumbPath = `thumbnails/${params.treatmentId}/${randomUUID()}.${thumbExt}`;
